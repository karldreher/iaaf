...
def test_archive_search():
    ...
    assert search.query == 'mediatype:audio AND item_size:[0 TO 1000000000000] AND title:"Kool and the gang"'
    search = ArchiveSearch(title="Parliment Funkadelic")
    assert search.query == 'mediatype:audio AND item_size:[0 TO 1000000000000] AND title:"Parliment Funkadelic"'
    search = ArchiveSearch(title="George Clinton", subject="Funk music")
    assert (
        search.query
        == 'mediatype:audio AND item_size:[0 TO 1000000000000] AND title:"George Clinton" AND subject:"Funk music"'
    )
